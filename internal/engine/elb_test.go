...
const elbTestPrefix = "AWSLogs/111122223333/elasticloadbalancing/us-east-1/2026/09/23/"

func elbKey(seg, suffix string) string {
	return elbTestPrefix + "111122223333_elasticloadbalancing_us-east-1_" + seg + "_20260923T0715Z_192.0.2.60_" + suffix
}

// albLine builds a minimal but complete ALB log line.
func albLine(ts, client, status, path string, latency float64) string {
	return fmt.Sprintf(`https %s app/tiles/98e3 %s:5815 10.0.20.37:80 0.001 %.3f 0.000 %s %s 153 3860 "GET https://tiles.example.com:443%s HTTP/1.1" "Dart/3.11" ECDHE-RSA-AES128-GCM-SHA256 TLSv1.2 arn:tg "Root=1" "tiles.example.com" "arn:cert" 2 %s "forward" "-" "-" "10.0.20.37:80" "200" "-" "-" TID_x "-" "-" "-"`,
		ts, client, latency, status, status, path, ts)
}

func elbScope() s3src.Scope {
	day := time.Date(2026, 9, 23, 0, 0, 0, 0, time.UTC)
	return s3src.Scope{Accounts: []string{"111122223333"}, Regions: []string{"us-east-1"}, Start: day, End: day}
}
...
func TestRunELBAggregatesAndFilters(t *testing.T) {
	...
	store := s3src.NewMemStore(map[string][]byte{
		...
		"AWSLogs/111122223333/CloudTrail/x.json.gz": []byte("out of scope"),
	})
...
func TestRunELBConnectionLogs(t *testing.T) {
	...
	connKey := elbTestPrefix + "conn_log_111122223333_elasticloadbalancing_us-east-1_app.k8s-web-0123456789.0123456789abcdef_20260923T0715Z_192.0.2.66_57hw2gm1.log.gz"
	store := s3src.NewMemStore(map[string][]byte{
		connKey:                              gz(t, conns),
		elbKey("app.tiles.98e3", "a.log.gz"): gz(t, albLine("2026-09-23T07:15:47.000000Z", "198.18.71.40", "200", "/", 0.003)),
	})
	...
	res, _ = RunELB(context.Background(), store, ELBOptions{Scope: elbScope(), Filter: elblog.Filter{ClientIP: "198.18."}})
